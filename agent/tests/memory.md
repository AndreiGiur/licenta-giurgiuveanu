...
| `test_google_oauth.py`  | 2 teste mock pentru `api_google_enroll`: happy path (verifica POST + `/agent/google-enroll` in URL + body cu `id_token`) + propagare `ApiError` cand backend-ul respinge tokenul. **Total agent: 32 teste (17 + 13 + 2).** |
