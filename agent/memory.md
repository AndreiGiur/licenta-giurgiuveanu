...
| `nmap_runner.py`           | **Construire CLI args nmap + executie subprocess STREAMING + NMAP_PROFILES.** `validate_cidr`, `validate_lan_target` (refuza public + subnet > `MAX_LAN_HOSTS=4096`), `build_nmap_args(..., profile)`, `run_nmap(targets, xml_out, profile, timeout_sec, progress_cb, log)`. **Real-time (2026-06-01):** `run_nmap` foloseste `subprocess.Popen` + flag `--stats-every 2s` (adaugat in profile args); citeste output-ul linie cu linie si parseaza progresul cu **`parse_nmap_stats_line(line) -> (percent, timp_ramas) | None`** (regex pe "About X% done" + "(... remaining)"), apeland `progress_cb(percent, remaining)`. **`NMAP_PROFILES`**: `advanced` (usor: `-sV -T4 --top-ports 1000`, fara `-O`/`-A`, doar scriptul `vulnwatch-audit`) + `deep` (agresiv `-A ... vuln,default,auth,banner`, **`all_ports=True` → `-p-` scaneaza toate cele 65535 porturi**, timeout 60min). Pe subnet `-Pn` e scos, deci `-p-`/`-A` ruleaza doar pe host-urile vii (fezabil). `build_nmap_args` foloseste `-p-` cand param `all_ports` SAU `prof["all_ports"]`. **Scan subnet (2026-06-01):** `build_nmap_args`/`run_nmap` au param `subnet_scan`; argumentele `single_host_args` din profil (ex. `-Pn` la deep) se aplica **doar pe tinta single-host** — pe un subnet le omitem ca nmap sa descopere host-urile vii intai (altfel `-A` ar rula pe toate cele 256 adrese). Profile `legacy` (default) = comportament vechi. **Fix duplicate script ID (2026-06-11):** cand `nse_script_path` e dat, `_scripts()` pune categoriile in expresie `((vuln or default or auth or banner) and not vulnwatch-audit)` — altfel copia din scripts dir (plantata de `deploy_nse_script()` la startup) era incarcata a doua oara prin categoria `vuln` si nmap facea abort instant pe TOATE scanarile deep (hosts=[], lua_errors). Exceptie: `NmapRunnerError`. |
...
| `gui.py`                   | **Interfata Tkinter cu paleta Ocean (albastru + teal; dark + light, toggle persistat) — oglindeste FE/PDF (2026-06-01).** `THEME_DARK`/`THEME_LIGHT` cu aceleasi chei (`bg/surface/elevated/border/accent/accent_hover/text/text_dim/text_muted/green/amber/red`); butonul Accent are text alb pe fundal albastru in ambele moduri. (Fix typo: cheia paletei e `text_muted`, nu `muted`.) **Installer nmap (2026-06-01):** bifa opt-in (default debifat) "Instaleaza nmap" pe pagina Enroll (`_var_install_nmap`); la finalizarea enrollment-ului `_maybe_install_nmap()` ruleaza `core.install_nmap` pe thread daca e bifata. Buton fallback "Instaleaza nmap" in sectiunea Detalii din Status cand `_nmap_path()` e None. Logging thread-safe prin `log_queue` (`_run_nmap_install`). 3 pagini: **Login** (Google button outlined + email/parola + toggle Register + footer doar cu versiunea + theme toggle ☾/☀), **Enroll consolidat** (sub-stari `new device` vs `relink` in aceeasi pagina, banner contextual cand exista UID, link `[Schimbă]` UID pe new device, link `Vrei să-l înregistrezi ca PC nou? →` pe relink), **Status** (status dot 5 stari `online/degraded/offline/paused/starting` cu glow live + 3 metric cards `SCANĂRI/ULTIMA EXPUNERE/ULTIMA SCANARE` + sectiune Detalii expandabila (state persistat in `[ui] log_expanded`) + meniu ⚙ cu **Pornește la logon / Schimbă cont / Deconectează acest PC / Despre**). Helpers: `ThemeManager` (toggle + persist), `_make_theme_toggle_button`, `_tick_status_refresh` (refresh la 2s pe baza `daemon.last_heartbeat_ts`), `_format_last_scan_time`, `_build_details_section`, `_render_status_dot`, `_open_about_dialog`, `_open_settings_menu`. **(2026-06-08) API ascuns complet din UI**: endpoint-ul backend e fix (`core.DEFAULT_API_BASE`, configurat in executabil) si NU mai apare nicaieri in interfata — eliminate footer-ul API + iconita ✎, modalul `_open_api_url_modal`, helperul `_refresh_api_short`, intrarea „Setări avansate API URL" din ⚙ si linia „API:" din sectiunea Detalii. `_var_api` ramane doar ca holder intern (login/google/relogare), neexpus. `_on_change_account` (păstrează device pe cont + păstrează metrici) vs `_on_disconnect_pc` (resetează metrici). Google login flow async pe thread separat ca inainte. Daemon ruleaza pe thread separat (`DaemonRunner`) cu callbacks `on_heartbeat_ok` (updateaza `last_heartbeat_ts`) + `on_scan_done` (trimite `__SCAN_DONE__` pe queue → `_poll_log_queue` apeleaza `metrics.record_scan` + re-render Status). |
| `tray.py`                  | Icon scut in system tray (`pystray` + `Pillow`). Meniu: **Open dashboard / Pauza / Iesire**. Optional — daca pystray lipseste, GUI-ul functioneaza fara icon. **Fix latin-1 (2026-06-02):** `_safe_title()` coercitioneaza titlul tray la latin-1 (backend-ul X11/Xlib codeaza titlul ferestrei ca latin-1 si crapa pe em-dash U+2014 etc.); aplicat la ambele atribuiri `_icon.title`. |
