...
| `build.sh`                 | **(2026-06-01) Build binar Linux** — oglinda lui `build.ps1`: venv `.venv-build`, instaleaza deps cross-platform (fara pywin32), `pyinstaller VulnWatchAgent.spec` → `dist/vulnwatch-agent`, `chmod +x`, copiaza in `server/app/static/agent/vulnwatch-agent`. ASCII pur, `set -e`. Binarul efectiv iese din CI (job `build-linux`) sau rulat pe Linux/WSL. |
