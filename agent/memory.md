...
| `gui.py`                   | **Interfata Tkinter, 3 pagini:** Login (buton **Continuă cu Google** deasupra + separator "sau" + form email/parola cu toggle inline pentru register), Enroll Device (cu smart re-link daca exista deja device cu acelasi UID), Status (cont + device + nivele suportate + buton **Deschide platforma** / Pauza / Logout; log live cu queue.Queue). **Fara buton "Scan now" — scanarea se initiaza din platforma web.** Google login flow async pe thread separat: `google_oauth.login_with_google()` → `core.api_google_enroll()` → `core.save_enrollment()` → pagina Status. Daemon ruleaza pe thread separat (`DaemonRunner`). **Sistem de teme Honey & Plum**: `THEME_DARK` + `THEME_LIGHT` dicts, `ThemeManager` class (persista in `config.ini` sub `[ui] theme`), `SEVERITY_COLOR_KEYS` dict, `severity_color()` helper. `AgentApp` foloseste `self.theme.palette["foo"]` in loc de globalul vechi `THEME["foo"]`. Toggle dark/light via `_on_theme_toggle()` / `_make_theme_toggle_button()`. |
