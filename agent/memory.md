...
| `core.py`                  | **Logica de baza, fara UI.** Citire/scriere config (`~/.vulnwatch/config.ini`), colectare date sistem (`is_admin`, `get_open_ports`, `get_processes`, `get_installed_software`, `collect_system_data`), apeluri HTTP (`api_login`, `api_register`, `api_create_device`, `api_get_device_by_uid`, `api_relink_device`, `api_send_scan`, `api_get_next_job`, `api_submit_job_result`, `api_submit_job_failure`, `api_logout`, `api_me`), bucla daemon (`daemon_loop`, `run_one_job`), helpere de enrollment (`login_or_register`, `enroll_device_with_session`, `perform_enrollment`), helpere PyInstaller (`is_frozen`, `executable_path`). |
...
| `gui.py`                   | **Interfata Tkinter, 3 pagini:** Login (cu toggle inline pentru register), Enroll Device (cu smart re-link daca exista deja device cu acelasi UID), Status (afiseaza cont + device, butoane Scan now / Pauza / Open dashboard / Logout, log live cu queue.Queue). Daemon ruleaza pe thread separat (`DaemonRunner`). |
