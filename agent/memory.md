...
| `nmap_runner.py`           | **Construire CLI args nmap + executie subprocess + NMAP_PROFILES.** `validate_cidr(value)` (sintaxa), `validate_lan_target(value)` (refuza public + subnet > `MAX_LAN_HOSTS=4096`), `build_nmap_args(targets, xml_out, top_ports, all_ports, extra_script_args, profile)`, `run_nmap(targets, xml_out, profile, timeout_sec, log)`. **`NMAP_PROFILES`** dict: `advanced` (moderat: `-sV -T4 --version-intensity 5 --top-ports 5000 --script vulnwatch-audit`, timeout 10 min) + `deep` (agresiv: `-A -T4 -Pn --version-intensity 9 --top-ports 5000 --script vulnwatch-audit,vuln,default,auth,banner`, timeout 40 min). Profile-ul `legacy` (default) pastreaza comportamentul vechi pentru backwards compat. Exceptie: `NmapRunnerError`. |
