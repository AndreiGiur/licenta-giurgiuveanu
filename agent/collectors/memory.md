...
| `system_info.py`    | `collect_system(cfg)` → `{system, release, version, machine, hostname, username, uptime_seconds, is_admin, firewall?, local_users?, bitlocker?, defender?, uac?, autologon?, smb1_enabled?}`. Firewall din registry; useri si Defender prin PowerShell. **local_users** contine `{name, enabled, password_required, is_admin}` — `password_required` din `Get-LocalUser PasswordRequired` (Task 2). **defender** include **`third_party_av`** lista din WMI `root\SecurityCenter2 AntiVirusProduct` cu real-time bit 0x1000. **uac** = `{enable_lua, consent_prompt_admin}` din registry. **autologon** = `{enabled, default_username, password_present}` (parola nu se citeste). **smb1_enabled** = bool|None din LanmanServer. |
