...
| `system_info.py`    | `collect_system(cfg)` → `{system, release, version, machine, hostname, username, uptime_seconds, is_admin, firewall?, local_users?, bitlocker?, defender?, uac?, autologon?, smb1_enabled?, password_policy?, audit_policy?}`. Firewall din registry; useri si Defender prin PowerShell. **local_users** contine `{name, enabled, password_required, is_admin}` — `password_required` din `Get-LocalUser PasswordRequired` (Task 2). **defender** include **`third_party_av`** lista din WMI `root\SecurityCenter2 AntiVirusProduct` cu real-time bit 0x1000. **uac** = `{enable_lua, consent_prompt_admin}` din registry. **autologon** = `{enabled, default_username, password_present}` (parola nu se citeste). **smb1_enabled** = bool|None din LanmanServer. **(Task 4) password_policy** = `{min_password_length, max_password_age_days, lockout_threshold}` (advanced+); **audit_policy** = `{audit_logon, audit_account_manage}` (deep) — ambele din `_secedit_export()` care ruleaza `secedit /export /areas SECURITYPOLICY` si scrie un INF UTF-16 LE; `_parse_secedit_inf(text)` parseaza INF-ul cu mapping fix de chei (locale-independent); `_to_int(val)` helper. Graceful degradation: daca secedit esueaza (ne-admin sau eroare), campurile lipsesc din output. |
