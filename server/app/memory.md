...
| `models.py`   | **6 tabele:** `User` (PBKDF2 salt + hash), `Session` (token sesiune cu expirare), `Device` (token hashed SHA-256, prefix de 8 char afisabil), `Scan` (payload JSON + exposure_score), `Finding` (legat de Scan, are rule_id + severity + evidence + recommendation), `ScanJob` (state machine `pending → running → done/failed/cancelled`). Constraint `uq_owner_device_uid` pe (owner_id, device_uid). Functie `hash_token(t)` pentru SHA-256. |
| `schemas.py`  | **Pydantic 2 schemas:** `RegisterIn`, `LoginIn` (cu `EmailStr`), `TokenOut`, `MeOut`, `DeviceCreateIn/Out` (cu `device_token` doar la creare), `ScanIn`, `ScanCreateOut`, `ScanDetailOut`, `DeviceScanListItem`, `ScanJobOut`, `AgentJobOut`, `JobResultIn`, `JobFailureIn`. Toate raspunsurile cu informatii despre device includ `device_uid` + `device_name`. |
