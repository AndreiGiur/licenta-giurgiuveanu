...
| `rules.py`   | **Motor de reguli cu decorator `@rule(id, min_level)`.** `evaluate(scan_dict) → (score, findings)` filtreaza automat regulile dupa `scan["scan_type"]` (LEVEL_ORDER: standard=0, advanced=1, deep=2). Adaugarea unei reguli noi = decoreaza o functie, zero modificari aiurea. **7 reguli standard** existente: `NET-OPEN-PORTS-1`, `NET-MANY-PORTS-2`, `OS-ADMIN-1`, `PROC-SUSPICIOUS-1`, `PROC-POWERSHELL-2`, `SW-VULNERABLE-1`, `OS-EOL-1`. Scoring: `100 * (1 - e^(-raw/60))`. Regulile noi (16) sunt adaugate in Task 5. |
