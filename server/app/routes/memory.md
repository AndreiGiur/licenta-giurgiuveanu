...
| `admin.py`     | `require_admin`: `GET /admin/users`, `DELETE /admin/users/{id}` (block self), `POST /admin/users/{id}/role` (block self-demote), `POST /admin/users/{id}/reset-password`, `GET /admin/devices`, `GET /admin/scans` (paginat), `GET /admin/stats`. **Optimizat N+1 (2026-06-10):** `list_users` (count device grupat), `list_devices` (emailuri owner batch), `list_scans` (device+owner batch pe pagina), `stats` (`func.avg` pe scor + `devices_online` calculat in SQL pe `last_heartbeat`, fara a incarca toate randurile in memorie). |
