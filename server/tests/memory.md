...
| `test_reports.py`               | 6     | Endpoint `GET /scans/{id}/report.pdf`: PDF valid (`%PDF-` signature, > 1KB), 404 pentru non-owner, admin bypass, sectiunea nmap creste dimensiunea PDF. **+2 (2026-06-01):** host nmap cu MAC+vendor+porturi detaliate+finding genereaza PDF mai mare (tabel retea); scan Linux cu `payload["linux"]` adauga sectiunea de audit. |
...
## Total: 380 teste server + 115 unit tests in `agent/tests/` + 74 teste frontend (Vitest) = **569 teste**.
