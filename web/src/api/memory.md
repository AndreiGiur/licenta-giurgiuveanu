...
| `auth.ts`      | Wrapper-i pentru endpoint-urile de auth: `registerUser`, `loginUser` (raspunsul contine `session_token` dar nu il folosim — cookie-ul e sursa de adevar), `fetchMe` (returneaza `Me` cu `google_picture_url?` + `auth_provider?`), `logoutUser`, `getGoogleAuthUrl` (returneaza `{auth_url, state}` pentru flow OAuth web). Frontend-ul **niciodata** nu citeste tokenul. |
