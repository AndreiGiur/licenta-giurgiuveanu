...
| `os.ts`        | **(2026-06-01)** `detectOS(ua?)` → `"windows" \| "linux" \| "other"` din user-agent (Android exclus din Linux desktop). Folosit de banner-ul de download din `Devices.tsx` ca sa ofere artefactul potrivit (.exe vs binar Linux). Tip `ClientOS`. Test: `os.test.ts` (4). |
| `auth.ts`      | Wrapper-i pentru endpoint-urile de auth: `registerUser`, `loginUser`, `fetchMe` (returneaza `Me` cu `google_picture_url?`, `auth_provider?`, **+ `role?`** "user"/"admin", **+ `first_name?` + `last_name?` + `default_scan_type?`**), `logoutUser`, `getGoogleAuthUrl`. **+ `updateMyProfile(input)`** wrapper pentru PATCH /me. Frontend-ul **niciodata** nu citeste tokenul. |
| `exposure.ts`  | Wrapper-i pentru scan-uri + device-uri: `listDeviceScans`, `getScan`, **`getScanPdfUrl(scanId)`** (URL absolut pentru export PDF), `requestScan(uid, scanType, nmapTarget?)`, `getScanJobPreview`, `getScanJob`, `listScanJobs`, `getAgentDownloadInfo`. **+ Schedule CRUD**: `listSchedules`, `createSchedule`, `deleteSchedule`. **+ `getDeviceScoreTrend(uid, days)` + `getScanDiff(scanId, previousId?)`** pentru graficul de trend si comparatie scan-uri. |
...
| `types.ts`     | **Tipuri TypeScript pentru raspunsurile API.** `ScanType`, `Finding`, `DeviceScanListItem`, `Device`, `ScanPayload` (+ `nmap`), `ScanDetailResponse` (+ `score_breakdown`), `ScanJobStatus`, `ScanJobResponse`, `ScanJobPreview`, `NmapHost/NmapData`. **+ `ScoreBreakdown`** (critical_risk/network_exposure/hygiene/activity) + constant `SCORE_CATEGORIES` (key + label RO + pondere) folosit de `<ScoreBreakdownBars>`. **+ `Schedule` / `ScheduleFrequency`** pentru scheduler. **+ Profile/Admin types**: `UserStats`, `SessionInfo`, `PlatformStats`, `AdminUserRow`, `AdminDeviceRow`, `AdminScanRow`, `AdminScansPage`. |
