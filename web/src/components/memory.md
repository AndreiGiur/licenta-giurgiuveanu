...
| `ScoreTrendChart.tsx` | **Recharts LineChart**: graficul de scor in timp pentru un device. Props: deviceUid + days + height. Fetch din `/devices/{uid}/score-trend?days=30`. **(2026-06-01) Refactorizat pe wrapper-ul `chart/`** (ChartContainer + ChartTooltip); gradient albastru→teal (Ocean), benzi de severitate cu `var(--severity-*)`. Empty/loading/error states. Folosit in Profile. |
| `chart/` (pachet) | **(2026-06-01) Wrapper grafice spirit-shadcn** peste Recharts (ChartContainer/ChartTooltip/ChartLegend + chart.ts). Vezi `chart/memory.md`. |
| `ScanDiff.tsx` | **Componenta diff intre 2 scan-uri (2026-05-20)**: 3 coloane (Adaugate/Rezolvate/Nemodificate) cu finding rows colorate dupa severitate. Header cu delta scor + clasificare (`improved` verde, `regressed` roz, `same` muted). Apel `/scans/{id}/diff?previous=`. Folosit in ScanDetail (sectiune toggle "Compara cu scanarea anterioara"). |
| `ConnectionTopology.tsx` | **(2026-06-01) Diagrama live Agent → Backend → Platform.** 3 noduri cu status dot (verde pulse online / gri offline); nodul Agent din `is_online`. Linii cu pachete animate (Framer Motion) cand `online \|\| scanActive` (flux de date). Props: `online`, `lastHeartbeat`, `scanActive`. Stiluri `.topo*` in index.css; respecta `prefers-reduced-motion`. Test: 3 teste. |
| `DeviceSidebar.tsx` | **(2026-06-01) Lista de dispozitive (master-detail).** Props: `devices` (`SidebarDevice[]`), `selectedUid`, `onSelect`. Dot online + nume (ellipsis) + scor badge + nr scanari; item activ evidentiat. Empty state. Folosit in `UnifiedDashboard`. Test: 3. |
| `DeviceWorkspace.tsx` | **(2026-06-01) Workspace device selectat.** Header: nume + status + selector tip scan + Scaneaza acum + Sterge. `<ScanTypeExplainer>` + progress bar job activ + monitor row (`ScoreGauge` 150 + `ConnectionTopology` + `NetworkTrafficChart`) + scanari (lista) + findings. Refoloseste `useScanJobPolling`, `useScanDetail`, `requestScan`, `apiDelete`, `listDeviceScans`. Test: 3. |
| `ScanTypeExplainer.tsx` | **(2026-06-01) Panou educativ** care arata live ce face tipul de scanare selectat (durata + ce colecteaza + nr reguli + faza nmap). Constanta exportata `SCAN_TYPE_INFO` (standard/advanced/deep). Font responsiv `clamp()`. Test: 3. |
| `InfoTip.tsx` | **(2026-06-08) Buton "i" + popover explicativ.** Props: `topic` (cheie in `help/helpContent.ts`) sau `title`+`body` inline; `size` (default 16). Click = fixeaza popover-ul (sticky); hover/focus pe desktop = preview temporar (cu delay de inchidere 140ms ca sa poti intra cu mouse-ul in popover). Inchidere prin Escape sau click in afara. Popover randat in **portal** (`createPortal` in `document.body`), pozitie `fixed` calculata din `getBoundingClientRect()` cu auto-flip sus/jos si clamp pe viewport; reasezare la scroll/resize. Accesibil: `aria-label`, `aria-expanded`, `aria-describedby`, `role="note"`. Framer Motion fade+scale; `prefers-reduced-motion` respectat. Stiluri `.infotip*` in `index.css`. **Nu reapela `focus()` pe buton la Escape** (declanseaza `onFocus`->redeschide). Test: `InfoTip.test.tsx` (4). Folosit in: DeviceWorkspace, ScoreBreakdownBars, ScanDetail, NmapSection/NmapHostCard, ScanDiff, Profile, Admin. |
| `NetworkTrafficChart.tsx` | **(2026-06-01) Grafic trafic live in/out.** Recharts AreaChart cu KB/s iesit (accent albastru) vs intrat (teal) pe ultimele ~10 min, prin `useNetworkTraffic` (poll 10s). **Refactorizat pe wrapper-ul `chart/`** (ChartContainer + ChartTooltip + ChartLegend); fix bug `--plum` nedefinit → `var(--color-in)`. Header cu ratele curente + nr conexiuni. Sursa: `psutil.net_io_counters()`. Test: 2. |
